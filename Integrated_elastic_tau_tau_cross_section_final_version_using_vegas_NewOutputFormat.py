...
## Save results to a text file with q2emax and q2pmax included in the filename
    #filename_txt = f"Elastic_Photon_Luminosity_Spectrum_q2emax_{int(q2emax)}_q2pmax_{int(q2pmax)}_using_vegas_NewOutputFormat.txt"
    #with open(filename_txt, "w") as file:
    ## Writing in the desired format
        #file.write(f"({q2emax}, {q2pmax}, {W_values.tolist()})\n")
        #file.write(f"({q2emax}, {q2pmax}, {[S_yy if S_yy is not None else 0.0 for S_yy in luminosity_values]})\n")

## Optional: Print a success message to confirm
        #print(f"Results have been saved to {filename_txt} in the specified format.")
...
# Save results to a Python script file in the desired format
    ...
    # Writing W_values
        ...
        file.write(f"elas = ({q2emax}, {q2pmax}, {[S_yy for S_yy in luminosity_values if S_yy is not None and S_yy != 0.0]})\n")
    # Writing elastic luminosity values

# Optional: Print a success message to confirm
        print(f"Results have been saved to {filename_py} in the specified format.")
