# The `integrated_higgsinos_cross_section` function computes the integrated mu-mu production cross-section 
...
def cs_higgsinos_w_condition_Hamzeh(W):
    alpha = 1 / 137.0
    ...
    if W < 2 * mhiggsino:
        return 0.0
    beta = math.sqrt(1.0 - 4.0 * mhiggsino**2 / W**2)
...
def integrated_higgsinos_cross_section(W0, eEbeam, pEbeam):
    s_cms = 4.0 * eEbeam * pEbeam  # Center-of-mass energy squared

    def integrand(W):
        # Get the mu-mu cross-section and S_yy value
        higgsinos_cross_section = cs_higgsinos_w_condition_Hamzeh(W)
        ...
        return higgsinos_cross_section * S_yy_value     # to calculates the integrated mu-mu cross-section

    try:
        result, _ = integrate.quad(integrand, W0, np.sqrt(s_cms), epsrel=1e-4)
    except integrate.IntegrationWarning:
        print(f"Warning: Integration for higgsinos production cross-section did not converge for W_0={W0}")
        result = 0.0
    except Exception as e:
        print(f"Error during integration for higgsinos production cross-section: {e}")
        result = 0.0
    return result
...
integrated_cross_section_value = integrated_higgsinos_cross_section(W0_value, eEbeam, pEbeam)
print(f"Integrated inelastic mhiggsino Production Cross-Section at W_0 = {W0_value} GeV: {integrated_cross_section_value:.6e} pb")
