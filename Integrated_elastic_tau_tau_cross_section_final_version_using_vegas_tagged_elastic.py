...
#  Hamzeh Khanpour, Laurent Forthomme, and Krzysztof Piotrzkowski -- November 2024 -- tagged elastic
...
def flux_y_proton(yp):
    if yp <= 0.01 or yp >= 0.20:                                 # Hamzeh tagged elastic   ==>  if (yp <= 0.01 or yp >= 0.20):
...
W_values = np.logspace(1.0, 2.874, 404)  # Range of W values from 10 GeV to 1000 GeV
...
if __name__ == "__main__":
    ...
    filename_txt = f"Elastic_Photon_Luminosity_Spectrum_q2emax_{int(q2emax)}_q2pmax_{int(q2pmax)}_using_vegas_tagged_elastic_LHeC750GeV.txt"
